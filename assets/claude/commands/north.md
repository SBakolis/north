...
5. When the CLI is available and the project has no OpenSpec directory, ask:
   "OpenSpec is installed. Add OpenSpec to this project with Claude Code support?"
   State the resolved project path and that this will run
   `openspec init --tools claude` there, creating OpenSpec project files and
   Claude Code integration files. Wait for an explicit answer. Declining or leaving
   the question unanswered keeps the North directory and skips initialization.
6. Only after the user accepts, run `openspec init --tools claude` with the
